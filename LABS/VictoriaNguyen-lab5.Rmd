---
title: "STAT167 Lab #5 - Spring 2022"
...
# 86,326 flights
# alternatively: filter(flights, month %in% c(7,8,9))
```
...
# the shortest flights are to PHL
```
...
# the shortest flights spent 20 minutes in the air, to BDL
```
...
```{r}
flights %>% 
  filter(!is.na(dep_delay), !is.na(arr_delay)) %>% # filter out canceled flights
  group_by(carrier) %>% # group by carrier
  mutate(delay = mean(arr_delay)) %>% # then new col,average arrival delay per carrier
  arrange(desc(delay)) # rank results

# F9 is the carrier with worst delays. 
```
